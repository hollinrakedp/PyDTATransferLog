...
2. **Add Files and Folders**:
   ...
   - Click the **Add Folders** button to select entire directories
   - Click the **Import Request** button to import files from a request file or file list
   - **Drag and Drop** files or folders directly into the file list area
   - The application will automatically scan and display the selected items

3. **Import Request Files**:
   - Use the **Import Request** button to streamline the transition from request to transfer
   - Supports multiple file formats:
     - **CSV files**: Request files generated by the application's request feature
     - **Text files**: Plain text files with one file path per line
     - **Auto-detection**: Files with unknown extensions are analyzed for format
   - Text file format features:
     - Comments supported (lines starting with `#` or `//`)
     - Quoted paths supported (`"path"` or `'path'`)
     - Empty lines automatically ignored
     - Cross-platform path handling
   - Import behavior:
     - Only file paths are imported (no metadata like requestor, purpose, etc.)
     - Missing files are reported but don't prevent import
     - Warning if already selected files are not in the selected request. There are options to keep all or remove non-request files or cancel the import.
...
PyDTATransferLog supports importing file lists to streamline the workflow from request creation to transfer logging. This feature supports multiple file formats and provides flexible options for managing file selections.

#### Supported File Formats

1. **Request CSV Files**: Structured CSV files generated by the application's request feature
   - Contains metadata and file information
   - Only level 0 files (top-level files) are imported, excluding archive contents

2. **Plain Text Files**: Simple text files with one file path per line
   - Supports comments (lines starting with `#` or `//`)
   - Handles quoted paths (`"path with spaces"` or `'single quotes'`)
   - Ignores empty lines automatically
   - Cross-platform path handling

3. **Auto-Detection**: Files with unknown extensions are automatically analyzed
   - Detects CSV format by looking for expected column headers
   - Falls back to text file parsing for unrecognized formats

#### Text File Format Example

```text
# Project files for transfer
C:\Documents\project_report.pdf
C:\Documents\data_analysis.xlsx
"C:\Program Files\Custom App\config.txt"
./relative/path/file.doc

# Network files
\\server\share\important_data.zip
...
1. **Access Import**: Click the **Import Request** button in the Log tab
2. **Select File**: Choose from Request Files (*.csv), Text Files (*.txt), or All Files (*)
3. **Handle Conflicts**: If you already have files selected, the application will:
   - Show a warning about mixed file sources
   - Provide three options:
     - **Keep All**: Import request files and keep existing selections
     - **Remove Files**: Remove existing files not in the request, then import
     - **Cancel**: Cancel the import operation
4. **Review Results**: The application will show:
   - Number of files successfully imported
   - List of missing files that couldn't be found (for reference)

#### Benefits of Import Functionality

- **Streamlined Workflow**: Easily transition from request creation to transfer logging
- **Flexibility**: Support for multiple file formats accommodates different workflows
- **Error Handling**: Missing files are reported but don't prevent import
- **Cross-Platform**: Works consistently on Windows and Linux systems
- **External Integration**: Other tools can generate simple text lists for import
